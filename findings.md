...
- 删除执行已改为后台线程，确认窗口在提交后会立刻关闭，顶部横幅、状态栏和 Inspector 会持续显示后台删除任务，避免大文件/大目录删除直接阻塞 UI 线程。
- Windows 回收站删除现已增加系统回收站二次校验，不再只凭 API 返回成功就视为已回收。
- 选中文件夹后，“最大文件”榜单能够切换到该目录上下文，直接展示这个文件夹内部最大的文件。
- 启动默认根路径不再是 `.`，并新增盘符快捷扫描按钮，降低首次使用门槛。
- `relay-1` 调试标记已从窗口标题、导航和扫描提示中移除，界面回到正式展示语义。
...
- 删除后台任务当前只能表达“系统正在处理”，仍缺少更细粒度的阶段或字节级进度，这会影响用户对长耗时删除的信任感。
