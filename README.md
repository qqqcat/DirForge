...
- `dist/DirOtter-windows-x64-0.1.0-portable/BUILD-INFO.json` 已记录当前打包时间、commit 和签名状态
- 当前本地产物状态：`NotSigned`
...
- 结果视图：扫描完成后默认用完成态 Top-N 目录和文件绘制轻量 Treemap，不自动载入完整 `NodeStore`，不再走实时重布局或重反序列化路径。
...
- 缺少自动化视觉回归，页面留白、壳体内边距、按钮对比度、Settings 下拉框、排名列表坐标、明暗主题和多语言布局仍主要依赖人工检查。
