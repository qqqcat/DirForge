...
DirOtter 是一个基于 Rust 的本地磁盘分析器原型，当前聚焦于：
...
- 整体品牌语气应该更像“冷静的分析工具”，而不是“激进的垃圾清理器”
...
- 删除执行已改为后台线程；确认窗口会先关闭，再切换到顶部横幅、状态栏和 Inspector 的后台任务提示，删除期间仍可继续浏览结果。
- Windows 回收站删除新增系统回收站二次校验，选中文件夹后“最大文件”榜单会切换到目录上下文。
- 启动时会优先选择系统盘/首个卷作为默认根路径，并提供盘符快捷按钮，点击即可直接扫描对应卷。
- Workspace `cargo check/test` 均可通过，含扫描、平台、报告、阈值测试回归。
- 仍需继续补强删除中的细粒度反馈、真实删除跨平台边界与长跑稳定性验证。
...
- UI 能力：支持中英文切换、系统中文字体回退、实时检查器、Stop Scan 动作、盘符快捷扫描、人类可读文件大小格式化、可滚动排行榜、目录上下文文件榜单与删除后即时局部刷新。
...
建议阅读顺序：`dirotter-scan` → `dirotter-core` → `dirotter-ui`。
