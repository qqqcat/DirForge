...
3. 保持默认 `推荐策略`；只有在排查复杂目录、外置盘或超大硬盘时，再展开 `高级扫描节奏`：
   - `复杂目录`
   - `外置/超大硬盘`
   - 这些节奏都会扫描同一范围并产出同一组结果，只改变批处理和界面刷新频率
...
- 设置保存在 `settings.json`
- 扫描结果默认只保留在当前会话内
- Duplicate Files 需要完整结果树时使用当前会话临时 `zstd+bincode` 快照
- 应用启动不再依赖 SQLite
