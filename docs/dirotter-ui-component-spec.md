...
- 卡片容器必须保留最小外边距，不能让描边直接贴住子 `Ui` 的 clip rect，否则右边框和下边框会出现裁切

## 3. 页面职责

### Overview

- 负责清理建议、卷空间摘要、最大文件夹、最大文件、扫描目标与盘符快捷扫描
- 第一排主区应以显式两列布局组织“扫描目标”和“卷空间摘要”
- 扫描目标卡必须优先展示用户模式，而不是技术参数
- 扫描完成后，Overview 顶部必须优先展示 `清理建议` 卡片，先回答“我能怎么快速腾出空间”
- 清理建议卡必须包含：
  - 可释放空间总量
  - 分类摘要
  - `查看详情`
