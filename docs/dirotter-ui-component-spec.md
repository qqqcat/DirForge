...
- 二级工具页面：Errors / Diagnostics（仅在 `高级工具 / Advanced Tools` 开启后显示）
- 已移除页面：Operations
...
- Inspector 也必须拥有独立纵向滚动；右栏新增卡片后不得依赖“刚好够高”来避免内容被底部状态栏吃掉
- `Errors / Diagnostics` 属于高级工具页，默认不应打断主清理路径
...
- Result View 不应自动载入历史旧缓存；只有当前会话已经有结果摘要时，才允许按需回载对应临时快照
- 只展示当前目录的直接子项，不递归整树
- 支持逐层进入下一层和返回上级
...
- `Release System Memory` 的结果摘要应固定放在 Inspector 内，不再把长说明文案塞进卡片顶部
- 300px 级别的 Inspector 窄栏中，内存状态卡不得依赖横向 chip 承载长标签；应优先使用纵向统计行，避免右侧裁切
- `优化 DirOtter 内存占用` 只能针对应用自身占用，不能误导成系统级提速承诺
- 删除完成后的结果同步不得为每个目标都复制整棵结果树；必须采用单次更新，避免 UI 卡死和峰值内存翻倍
- 当结果树被释放时，必须先有可恢复的当前会话临时快照，再允许在结果视图中按需回载
- 状态栏应保持紧凑全局态；系统可用内存、负载和最近一次释放结果应由 Inspector 内存状态卡承载
...
- 删除失败后应给出针对性的失败提示
- 删除完成后的结果同步不得在 UI 主线程里同步重建 `NodeStore` 或重算 cleanup analysis；若需要重建结果视图，必须进入独立后台同步阶段并给出可见的 `结果同步中` 状态
- 结果视图从快照恢复结果时，不得在页面渲染函数里同步读取临时快照、解压快照或重建结果树；若结果 `store` 当前未驻留内存，必须进入后台载入态，删除/结果同步期间应先显示等待提示
- 批量清理确认必须明确写清：项目数、预计释放量、默认走回收站
- 批量清理确认中的待处理项目必须提供可滚动完整列表，不允许只显示前几项或只给截断路径
- 失败原因的完整原文不得长期挤在 Inspector 外层摘要区；当失败项存在时，应通过可点击的失败统计入口打开详情卡片，集中展示完整路径、失败原因和建议
- 失败详情卡片必须受控宽度并使用全宽列表项布局，长路径要换行，不能让单个条目把窗口横向撑出屏幕
- 失败详情的显式关闭按钮必须放在顶部可见区，不能只放在滚动内容底部
- 多语言界面中，失败项主文案必须使用本地化失败标题与解释；原始技术错误串只能作为次级细节显示
