...
- 已实现：`eframe/egui` UI、当前会话临时快照恢复、页面级滚动、对称内容 gutter、轻量结果视图与列表展示、19 语言选择（其中中文/英文/法语/西班牙语为完整文案，其余新增语言当前回退英文 UI）。
- 已实现：默认根路径选择、盘符快捷扫描、三档扫描模式预设、Inspector 内真实删除（回收站/永久删除确认）、后台删除任务提示、目录上下文文件榜单、文本/CSV 报告模块。
- 已实现：系统快照与指标描述、路径可达性评估、当前会话诊断视图。
...
- **Session Storage Engine**：设置文件与当前会话临时快照管理。
- **Product Refocus Direction**：默认用户路径优先“扫描 -> 清理建议 -> 执行删除 -> 释放空间确认”；跨会话历史、诊断导出与数据库工作台能力已移出默认产品模型。
...
- **Maintenance Utilities**：普通用户的系统内存释放入口固定放在右侧 `Quick Actions` 的 `一键释放系统内存`；技术性维护下沉到 Diagnostics，包括 `优化 DirOtter 内存占用` 与“清理异常中断的临时删除区”。前者会优先把结果树写入当前会话临时快照，再释放应用内存并在需要时按需回载。
...
- **L3 轻量持久层**：`settings.json` 设置文件。
- **L4 会话临时恢复层**：`zstd+bincode` 临时快照，用于同一应用会话内释放/恢复结果树。
...
- **运行时观测**：telemetry 继续补充 `snapshot changed/materialized nodes` 与 `snapshot text bytes` 估算，诊断视图可直接判断 live snapshot 是否仍保持稀疏 payload。
...
- **Result View Snapshot Load 非阻塞化**：结果视图对会话快照的恢复也已迁到后台 session；页面切换不允许在渲染期同步执行快照读取、zstd 解压、`NodeStore` 反序列化或 cleanup analysis 重建。删除/结果同步期间若结果 `store` 未驻留内存，结果页必须先显示等待态。
...
- 快照策略：只在需要释放应用内存时按需写入当前会话临时快照；默认不在每次扫描结束后自动写入。
