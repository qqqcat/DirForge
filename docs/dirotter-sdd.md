...
- 默认入口只暴露 `推荐策略`
- `复杂目录` 和 `外置/超大硬盘` 保留在高级扫描节奏中

所有节奏都会完整扫描当前范围并产出同一组结果，差异只在批处理和界面刷新频率。
...
- 删除完成后的结果同步在后台进行
- 独立 Result View / Treemap 入口已删除；完成态结果证据由 Overview 与 Inspector 承担
...
- Duplicate Files 需要完整结果树时才后台载入当前会话快照，渲染路径不得同步解压或反序列化
