...
- `dist/DirOtter-windows-x64-1.0.2-portable/BUILD-INFO.json`

## 2. 总体结论

DirOtter 当前处于 **Production Readiness with UI Regression Risk**。
...
- 工程主路径已经成立：`fmt + check + test + clippy -D warnings + build` 是当前合入门槛。
...
- 本地已存在 `1.0.2` 的 Windows 便携包与校验文件。
...
- 目前仍缺最小截图回归或视觉 diff。
- 首页栅格、结果页高度、多语言撑开、左侧导航贴边、底部状态栏贴边、Settings 下拉框和右侧 Inspector 按钮对比度仍主要靠人工检查。
- 2026-05-02 的截图回归暴露出同一类问题：壳体内边距、页面宽度、低级绘制坐标和按钮文字/背景对比没有被同一个门禁覆盖。
...
- 壳体类 UI 必须按最终 surface 检查：左侧导航、顶部工具栏、右侧 Inspector、底部状态栏和中央页宽都要同时看明暗主题。
- 危险按钮、主按钮和禁用按钮必须显式检查文字/背景对比，不能只依赖全局主题色。
