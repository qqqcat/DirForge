...
> 2026-04-09 更新：当前实现已完成“默认无数据库”收口。旧版文中关于 SQLite 承担设置、历史、审计和诊断导出的表述，现统一以“轻量设置文件 + 当前会话临时快照”替代。
>
> 2026-04-09 更新 2：发布门槛补齐了正式 CI、Windows 发布打包链路和可选代码签名入口；`dirotter-testkit` 的 massive tree snapshot 基线也已去掉 25ms 调度型脆弱等待。
...
- **扫描收尾已开始异步化**：目录遍历结束后，最终结果整理与建议汇总转到后台整理阶段，避免最后一拍把窗口卡成无响应。
- **存储模型已明显收口**：自动快照落库、历史写入和错误导出已从默认产品中移除，设置与恢复只保留轻量文件和当前会话临时快照。
...
- **启动链路已去数据库化**：应用不再因 SQLite 初始化失败而影响启动，设置与会话恢复分离后更符合“扫描即用、关闭即走”的产品模型。
...
- **发布门槛已从手工检查前移到自动化**：仓库现已提供正式 CI 和 Windows 发布 workflow，不再只依赖本地人工构建。
- **交付形态已不再局限源码构建**：Windows 便携发布包、SHA-256 校验文件和安装脚本都已进入仓库。
...
- **会话恢复热路径仍有优化空间**：临时快照保存与恢复已经脱离数据库，但大结果树下的写入时机和回载策略仍可继续收口。
...
- `.github/workflows/ci.yml`：已接入 `template validation + fmt + check + clippy + test + release build`
- `.github/workflows/release-windows.yml`：已接入 `tag/manual release + optional signing + portable packaging + checksum`
- `scripts/package-windows.ps1`：已提供 Windows 便携包生成逻辑
- `scripts/sign-windows.ps1`：已提供基于 secrets 的可选签名逻辑
...
- 轻量设置文件承担语言、主题和高级工具开关等偏好。
- 当前会话临时快照承担“释放应用内存后恢复结果树”的职责。
- 当前评价：存储层已更贴近产品定位，启动风险下降，但 UI 仍需继续减重。
...
- `Errors / Diagnostics` 已降级为二级高级工具入口
...
| 生成文件污染静态检查 | 低 | 已修复翻译生成文件中的隐形字符，并已把 `fmt / check / test / clippy` 固化到 CI | 继续为翻译生成链路补更明确的回归 |
| Windows 签名 secrets 未配置 | 低 | 发布链路支持可选签名，但默认允许无签名产物继续打包 | 在正式对外分发前配置生产证书并验签 |
...
- 发布门槛方面，本轮还补上了此前缺失的自动化链路：
  - `benchmark_thresholds.rs` 中的大树 snapshot payload 阈值测试不再依赖 25ms 的事件等待窗口
  - `ci.yml` 负责稳定执行 `fmt / check / clippy / test / release build`
  - `release-windows.yml` 负责生成便携 zip、SHA-256 校验文件，并在配置 secrets 时自动签名
