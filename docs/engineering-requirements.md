...
- 所有用户可见 UI 文案必须走 `app.t(...)` / `self.t(...)` 或等价的多语言 view-model 路径。
- 新增或修复文案时，必须补齐所有可选语言；禁止只补中文/英文/法语/西语，再让其他语言回退英文。
- 禁止用“英文 fallback 表”让翻译覆盖测试通过。缺失 key 必须补对应语言的真实翻译。
- 处理“单一语言硬编码”问题时，先追踪最终 UI writer，再补自动化门禁，证明非英文语言不会渲染 raw English key。
