# DirOtter Quick Start（2026-03-18）
...
6. 选中文件夹后，右侧“最大文件”榜单会切换为该目录内部的大文件。
7. 选中一个文件或文件夹，在右侧 Inspector 中直接执行“移到回收站”或“永久删除”。
8. 永久删除会先出现确认层；点击确认后窗口会立即关闭，并转为顶部横幅、状态栏和 Inspector 的后台任务提示。
9. 确认删除后榜单、概览统计与 treemap 已即时刷新。
10. 导出报告与诊断文件。
